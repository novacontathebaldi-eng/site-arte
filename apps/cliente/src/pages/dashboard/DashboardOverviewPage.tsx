...
import { PackageIcon, EuroIcon, HeartIcon, TruckIcon } from '@shared/components/ui/icons';
...
const DashboardOverviewPage: React.FC = () => {
    ...
    const formatDate = (dateString: any) => {
        if (!dateString?.seconds) return '...';
        return new Date(dateString.seconds * 1000).toLocaleDateString(language, { year: 'numeric', month: 'long', day: 'numeric' });
    };

    return (
        <div className="space-y-8">
            <h1 className="text-3xl font-bold text-primary">
                {t('dashboard.overviewTitle')} {user?.profile?.displayName || user?.email}!
            </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <StatCard icon={TruckIcon} title={t('dashboard.activeOrders')} value={recentOrders.filter(o => o.status !== 'delivered' && o.status !== 'canceled').length} />
                    ...
                    <div className="space-y-4">
                        {recentOrders.map(order => (
                            <div key={order.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 border rounded-lg hover:bg-surface transition-colors">
                                <div>
                                    <p className="font-semibold text-primary">#{order.id.substring(0,6)}</p>
                                    <p className="text-sm text-text-secondary">{formatDate(order.createdAt)}</p>
                                </div>
                                <div className="flex items-center gap-4 mt-2 sm:mt-0">
                                    <p className="font-semibold">{formatCurrency(order.totals.totalCents / 100)}</p>
