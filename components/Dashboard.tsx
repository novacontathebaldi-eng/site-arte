...
export const Dashboard: React.FC = () => {
    ...
    // --- PROTEÇÃO DE ESTADO ---
    // Se o Dashboard estiver aberto mas o usuário for null (logout), fecha imediatamente.
    useEffect(() => {
    ...
    const handleLogout = async () => {
        closeAllOverlays(); // Fecha UI imediatamente
        await logout(); // Processa logout no Firebase
    };
    ...
    // Render Condicional Estrito: Só renderiza o conteúdo se estiver aberto E tiver usuário
                    ...
                    >
                        ...
                        <div className="flex-1 flex flex-col h-full overflow-hidden relative bg-[#121212]">
                            ...
                            <div className="h-24 border-b border-white/10 flex justify-between items-center px-8 md:px-12 bg-[#1a1a1a]/50 backdrop-blur shrink-0">
                                ...
                                <button onClick={closeDashboard} className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-colors">
                                    <X size={20} />
                                </button>
                            </div>
                                            ...
                                            <div>
                                                {loadingOrders ? (
                                                    ...
                                                    <div className="text-center py-24 border-2 border-dashed border-white/10 rounded-xl bg-white/5">
                                                        ...
                                                        <button onClick={closeDashboard} className="text-accent uppercase tracking-widest text-sm font-bold hover:underline">Ir para a Loja</button>
                                                    </div>
                                                ) : (
                                                ...
                                                {!isAddressFormOpen ? (
                                                    ...
                                                    >
                                                        ...
                                                        <form onSubmit={handleSaveAddress} className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
                                                            {/* Address Form Inputs - Mantidos iguais */}
                                                            ...
                                                            <InputField label="Rótulo (Ex: Casa, Trabalho)" value={addressForm.name || ''} onChange={(e:any) => setAddressForm({...addressForm, name: e.target.value})} placeholder="Casa" required />
                                                            <InputField label="Nome do Destinatário" value={addressForm.recipientName || ''} onChange={(e:any) => setAddressForm({...addressForm, recipientName: e.target.value})} placeholder="Nome completo" required />
                                                            <div className="md:col-span-2"><InputField label="Endereço (Rua, Número)" value={addressForm.line1 || ''} onChange={(e:any) => setAddressForm({...addressForm, line1: e.target.value})} placeholder="Rue de la Gare, 10" required /></div>
                                                            <div className="md:col-span-2"><InputField label="Complemento (Opcional)" value={addressForm.line2 || ''} onChange={(e:any) => setAddressForm({...addressForm, line2: e.target.value})} placeholder="Apt 4B" /></div>
                                                            ...
                                                            <InputField label="Estado / Província" value={addressForm.state || ''} onChange={(e:any) => setAddressForm({...addressForm, state: e.target.value})} required />
                                                            <div className="mb-4">
                                                            ...
                                                            <div className="md:col-span-2 pt-4 flex gap-4">
                                                                <button type="submit" className="flex-1 bg-accent text-white py-3 font-bold uppercase tracking-widest text-xs rounded hover:bg-accent/80 transition-colors">Salvar Endereço</button>
                                                                <button type="button" onClick={() => setIsAddressFormOpen(false)} className="px-6 border border-white/10 text-white rounded hover:bg-white/5 text-xs uppercase tracking-widest">Cancelar</button>
                                                            </div>
                                                        </form>
                                                    </motion.div>
                                                )}
                                                ...
                                                <div className="bg-[#1a1a1a] p-8 rounded-xl border border-white/10 space-y-6">
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-6">
                                                            <AvatarUploader />
                                                            <div className="text-sm text-gray-500"><p>JPG, GIF ou PNG. Max 2MB.</p></div>
                                                        </div>
                                                    </div>
                                                    <div className="grid grid-cols-1 gap-6">
                                                        <div>
                                                            <label className="block text-gray-500 text-xs uppercase tracking-widest mb-2">Nome de Exibição</label>
                                                            <input type="text" disabled={!isEditingProfile} value={profileForm.displayName} onChange={e => setProfileForm({...profileForm, displayName: e.target.value})} className="w-full bg-[#121212] border border-white/10 rounded p-3 text-white disabled:opacity-50 disabled:cursor-not-allowed focus:border-accent focus:outline-none" />
                                                        </div>
                                                        <div>
                                                            <label className="block text-gray-500 text-xs uppercase tracking-widest mb-2">Email</label>
                                                            <input type="email" disabled value={user.email} className="w-full bg-[#121212] border border-white/10 rounded p-3 text-gray-400 cursor-not-allowed" />
                                                        </div>
                                                        <div>
                                                            <label className="block text-gray-500 text-xs uppercase tracking-widest mb-2">Telefone Principal</label>
                                                            <input type="tel" disabled={!isEditingProfile} value={profileForm.phoneNumber} onChange={e => setProfileForm({...profileForm, phoneNumber: e.target.value})} placeholder="+352 ..." className="w-full bg-[#121212] border border-white/10 rounded p-3 text-white disabled:opacity-50 disabled:cursor-not-allowed focus:border-accent focus:outline-none" />
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
