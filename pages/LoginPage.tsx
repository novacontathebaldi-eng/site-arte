...
const LoginPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Error is handled by AuthContext, just stop loading
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="flex items-center justify-center min-h-[70vh] bg-surface">
            <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
                <h1 className="text-3xl font-serif font-bold text-center text-primary">{t('login_to_account')}</h1>
                ...
                <p className="text-center text-sm text-text-secondary">
                    {t('dont_have_account')} <Link to="/register" className="font-medium text-secondary hover:underline">{t('register')}</Link>
                </p>
            </div>
        </div>
    );
