...
interface OrderCardProps {
    order: Order;
    // FIX: Corrected the type for the payload to include pickupTimeEstimate from the Order type.
...
export const OrderCard: React.FC<OrderCardProps> = ({ order, onUpdateStatus, onUpdatePaymentStatus, onUpdateReservationTime, onDelete, onPermanentDelete, isSelectable, isSelected, onSelect }) => {
    ...
    // FIX: Corrected property access for reservationTime.
    ...
    const handleAccept = () => {
        ...
        // FIX: Allowed 'reserved' as a valid OrderStatus.
    ...
    // FIX: Corrected comparison for `completed` status.
    ...
    // FIX: Correctly accessed 'neighborhood' and 'address' properties.
                ...
                <div className={`flex-grow relative bg-white rounded-lg shadow-md border-l-4 ${config.color} overflow-hidden transition-opacity ${isArchived || status === 'deleted' ? 'opacity-70' : ''}`}>
                    <div className="p-4">
                        ...
                        {items && items.length > 0 && (
                            <div>
                                ...
                                <ul className="space-y-1 text-sm">
                                    {/* FIX: Used `item.id` for the key and changed `item.size` to `item.dimensions` to match the updated `Order.items` type. */}
                                    {items.map(item => (<li key={item.id} className="flex justify-between p-2 bg-gray-50 rounded"><span>{item.quantity}x {item.name} ({item.dimensions})</span><span>{(item.price * item.quantity).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></li>))}
                                </ul>
                            </div>
                        )}
                        ...
                        <div className="flex flex-wrap items-center justify-end gap-2 mt-4 pt-4 border-t">
                            {status === 'deleted' ? (
                                 <>
                                    {/* FIX: Changed status update to 'completed' which is a valid OrderStatus. */}
