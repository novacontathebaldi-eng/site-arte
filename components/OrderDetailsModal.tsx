...
export const OrderDetailsModal: React.FC<OrderDetailsModalProps> = ({ order, onClose, title }) => {
    ...
    // FIX: Correctly accessed 'neighborhood' property.
    ...
    const CompletedStatusBanner = () => {
        // FIX: Corrected comparison for `completed` status which is now part of OrderStatus.
                 ...
                 <div className="overflow-y-auto p-4 sm:p-6 space-y-4">
                    ...
                    <div className={`grid grid-cols-1 ${!isReservation ? 'md:grid-cols-2' : ''} gap-4 text-sm`}>
                        <div className="bg-gray-50 p-3 rounded-md border">
                            ...
                            {isReservation && (
                                <>
                                    {/* FIX: Corrected property access for reservation details on the Order and OrderCustomerDetails types */}
                    ...
                    {order.items && order.items.length > 0 && (
                        <div>
                            ...
                            <ul className="space-y-1 text-sm">
                                ...
                                {order.items.map(item => (<li key={item.id} className="flex justify-between p-2 bg-gray-50 rounded"><span>{item.quantity}x {item.name} ({item.dimensions})</span><span className="font-semibold">{(item.price * item.quantity).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></li>))}
                            </ul>
                        </div>
                    )}
