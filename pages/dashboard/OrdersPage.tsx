...
const OrdersPage: React.FC = () => {
    ...
    const formatDate = (dateString: any) => {
        ...
        return new Date(dateString.seconds * 1000).toLocaleDateString(language, {
            year: 'numeric',
            month: 'long',
            day: 'numeric',
        });
                ...
                <div className="overflow-x-auto">
                    ...
                    <table className="min-w-full divide-y divide-gray-200 hidden md:table">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {orders.map(order => (
                                <tr key={order.id}>
                                    <td className="px-6 py-4 whitespace-nowrap font-medium text-primary">#{order.id.substring(0,6)}</td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold">{formatCurrency(order.totals.totalCents / 100)}</td>
                        ...
                        {orders.map(order => (
                            <div key={order.id} className="bg-surface p-4 rounded-lg shadow">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="font-bold text-primary">#{order.id.substring(0,6)}</p>
